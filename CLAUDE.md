...
- **Hay dos proyectos, y el del repo es siempre el de desarrollo.** El ref que aparece escrito en este archivo, en `.env.example` y en `.mcp.json` es el de **Dev** (`nlfwqnmidfdohuyhklqp`); producción vive en un proyecto aparte cuyo ref **no** se versiona. Producción se migra y se administra **a mano**, siguiendo `docs/produccion/runbook.md` y su `panel-checklist.md`, y **Claude no la toca**: ni por MCP, ni por CLI, ni por `psql`. Eso no es una convención sino una regla ejecutada: el hook `PreToolUse` de `.claude/hooks/guard-prod.sh` aborta cualquier llamada que nombre un ref de Supabase distinto del de Dev, y también `supabase link`, `db push`, `db reset`, `config push`, `psql`, `pg_dump` y `pg_restore` —los verbos del runbook son del humano, y se le pasan para que los corra con el prefijo `! `—. Ignora a propósito lo que vaya dentro de un heredoc, porque redactar el runbook es escribir esos comandos y no ejecutarlos. El ref de producción se guarda en `.claude/prod-ref.txt`, que git ignora, y sirve para que el hook lo bloquee también por nombre. Dos cosas que no se deducen: una migración llega a producción **siempre después** de aplicarse y verificarse en Dev, y se aplica con `db push --db-url` y no con `link`, porque `link` reescribe `supabase/.temp/project-ref` y entonces `npm run supabase:types` regeneraría los tipos contra producción sin avisar.
- **Todo cambio en la base de datos entra por una migración, y no hay segunda vía.** Tocar el esquema o los datos de Dev —tabla, columna, política, permiso, vista, función o filas— es escribir `supabase/migrations/<timestamp>_<slug>.sql` y aplicarlo con `npx supabase db push`. **Nada de `apply_migration` ni de `execute_sql` por MCP, y nada de SQL tecleado en el panel**: el MCP es para consultar e inspeccionar. El motivo no es de estilo sino de destino: producción es el otro proyecto y se migra a mano con el runbook, así que un cambio que sólo existe en el servidor de Dev no tiene manera de llegar allí —no está en git, no está en el orden de aplicación y no se puede repetir—. La migración es lo único que viaja de un proyecto al otro. Y se corrige **hacia delante**: un error no se deshace en el servidor, se arregla con una migración nueva encima —SPEC 07 no revirtió la siembra de SPEC 06, añadió una que la borra—. Ojo con lo que **no** cabe en una migración: los ajustes del panel (Site URL, URLs de redirección, proveedores, plantillas de correo, política de contraseña y rate limit) se replican a mano en cada proyecto, y por eso existen `docs/produccion/panel-checklist.md` y el espejo de `supabase/config.toml`.
...
- **Las pantallas que leen la base de datos se renderizan en cada visita**, y desde
  ...
  inventadas.
- **Las marcas y el catálogo se cambian con migraciones como todo lo demás**: la
  regla entera está en «Supabase», y de ahí sale que SPEC 07 no revirtiera la
  siembra de SPEC 06 sino que añadiera una migración que la borra.

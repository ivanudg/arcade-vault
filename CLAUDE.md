...
- **`.claude/agents/skin-designer.md`** es el subagente que se ocupa del **vestido** de las máquinas, y es transversal a la cadena anterior: no decide qué entra ni qué alcance tiene, sino que comprueba que cada motor de `ENGINES` tenga sus **tres skins obligatorias** —`clasico` (la paleta que el motor ya tiene hoy, extraída del código y no rediseñada), `neon` (sólo tokens `--av-*`) y `retro` (fósforo verde monocromo, donde las entidades se distinguen por brillo y no por tinte)— y diseña hex por hex las que falten. Lo que lo hace útil es que **inventaria las ranuras de color leyendo el código**, incluidas las que no están en `constants.ts`: los cinco literales sueltos de `asteroids/entities.ts`, el `"#000"` de fondo o el brillo de `tetris/board.ts`, que una auditoría a ojo se deja. Valida con las ocho reglas de `.claude/skin-designer/contrato-skin.md` y anota cada skin en su ledger `.claude/skin-designer/skins.md`, con el mismo trato que la memoria de `game-planner`: se versiona, el código manda sobre él y es el único archivo que el agente escribe. **No abre un `constants.ts` para editarlo**: para en el diseño y cierra con handoff. Hoy ese handoff es a `/spec`, no a `/spec-impl`, porque **el sistema de skins no existe** —`mount(canvas, cb)` no recibe tema y no hay selector—, así que su primera ronda da «cero de cuatro vestidas» y eso es un hallazgo, no un fallo.
...
cuando hay que rediseñar una paleta. Audita el código, diseña lo que falte, lo anota en
`.claude/skin-designer/skins.md` y para; el color no llega a un `constants.ts` sin pasar por
una spec.

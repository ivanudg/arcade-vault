...
const LABEL =
  "flex flex-col gap-2 text-[12px] tracking-av-wide text-av-text-muted";
...
export function AuthPanel() {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    // Sin usuario escrito, la sesión entra como JUGADOR.
    login(name || "JUGADOR");
    setName("");
    setEmail("");
    setPassword("");
  ...
  return (
    <div className="w-[min(100%,440px)] border border-av-cyan/28 bg-[rgba(13,15,22,0.92)] p-[clamp(22px,4vw,36px)] shadow-[0_0_44px_rgba(0,245,255,0.12)]">
      <div className="text-center font-display text-[15px] tracking-av text-av-cyan [text-shadow:0_0_12px_rgba(0,245,255,0.8)]">
        ARCADE
        <span className="text-av-magenta [text-shadow:0_0_12px_rgba(255,0,110,0.8)]">
          {" "}
          VAULT
        </span>
      </div>
        ...
        <div>
          <div className="grid grid-cols-2 border border-av-cyan/25">
            <button
              ...
              className={`cursor-pointer border-none px-2 py-3.5 font-display text-[9px] tracking-av active:scale-97 ${
                isRegister
                  ? "bg-transparent text-av-text-muted"
                  : "bg-av-cyan text-av-bg"
              }`}
            >
              INICIAR SESION
            </button>
          ...
          <form onSubmit={submit} className="mt-6 flex flex-col gap-4">
            ...
            <p className="mt-1 text-[11px] tracking-av text-av-line">
              Sesión simulada en este dispositivo. Conectar aquí: POST
              /api/auth/login · OAuth Google/GitHub vía Supabase.
            </p>
          </form>
        </div>
      )}

/**
 * El cuerpo de la pantalla 404, sin cabecera ni pie.
 *
 * Existe porque hay **dos** `not-found` y tienen que decir lo mismo: el de la
 * raíz de `app/`, que atiende las URLs que no corresponden a ninguna ruta y
 * monta cabecera y pie por su cuenta, y el de `app/(vault)/`, que atiende los
 * `notFound()` lanzados desde las páginas del grupo y los hereda de su layout.
 * Escribirlo dos veces sería la forma segura de que un día dijeran cosas
 * distintas.
 *
 * Es la única pantalla que no existe en references/templates/, así que se
 ...
 *
 * El acento es magenta porque en este producto magenta es el color de aviso: es
 * el que usa "NO HAY JUEGOS PARA ESA BUSQUEDA" en la biblioteca.
 */
    ...
    <main className="flex flex-1 items-center justify-center px-[clamp(14px,3vw,40px)] py-[clamp(44px,10vw,110px)]">
      <section className="flex max-w-[52ch] flex-col items-center gap-5 text-center animate-av-fade">
        ...
        <p className="text-[15px] leading-[1.75] text-pretty text-av-text-soft">
          Esa dirección no corresponde a ninguna máquina del vault. Vuelve a la biblioteca para ver
